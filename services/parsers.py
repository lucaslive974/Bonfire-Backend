...
def parse_csv_infracoes(csv: Any) -> pd.DataFrame:
    ...
    try:
        ...
        data_frame["DAT_OCOR_INFR"] = pd.to_datetime(
            data_frame["DAT_OCOR_INFR"], format="%d/%m/%Y %H:%M"
        )
        data_frame["DAT_EMIS_NOTF"] = pd.to_datetime(
            data_frame["DAT_EMIS_NOTF"], format="%d/%m/%Y"
        )
        data_frame["DAT_LIMT_RECU"] = pd.to_datetime(
            data_frame["DAT_LIMT_RECU"], format="%d/%m/%Y"
        )
        ...
        if "DAT_CANC" in data_frame.columns and not bool(
            ...
            data_frame["DAT_CANC"] = pd.to_datetime(
                data_frame["DAT_CANC"], format="%d/%m/%Y"
            )
...
def parse_xls_infracoes(xls: Any) -> pd.DataFrame:
    ...
    try:
        ...
        raise ErrReadingFile(
            f"Problema ao processar o arquivo no Load: {xls}. {e}", 500
        )
    ...
    try:
        ...
        data_frame["DAT_OCOR_INFR"] = pd.to_datetime(
            data_frame["DAT_OCOR_INFR"], format="%Y-%m-%d %H:%M:%S"
        )
        data_frame["DAT_EMIS_NOTF"] = pd.to_datetime(
            data_frame["DAT_EMIS_NOTF"], format="%Y-%m-%d"
        )
        data_frame["DAT_LIMT_RECU"] = pd.to_datetime(
            data_frame["DAT_LIMT_RECU"], format="%Y-%m-%d"
        )
        if "DAT_CANC" in data_frame.columns and not bool(
            data_frame["DAT_CANC"].isnull().all()
        ):
            data_frame["DAT_CANC"] = pd.to_datetime(
                data_frame["DAT_CANC"], format="%Y-%m-%d"
            )
        ...
        raise ErrReadingFile(
            f"Problema ao corrigir datas e manipular colunas: {xls}. {e}", 500
        )
